...
struct TreeNode {
  ...
  void resolve() {
    ...
    // required
    if (left != NULL) left -> apply(lazy);
    if (right != NULL) right -> apply(lazy);
    lazy.clear();
  }
};
...
class SegmentTree {
  ...
  long long getRangeSum(Node* cur, int l, int r, int ll, int rr) {
    ...
    return getRangeSum(cur -> left, l, m, ll, rr) +
      getRangeSum(cur -> right, m, r, ll, rr);
  }
  ...
  void applyRange(Node* cur, int l, int r, int ll, int rr, const Lazy &operation) {
    cur -> resolve();
    if (rr <= l || ll >= r) return;
    if (ll <= l && rr >= r) {
      cur -> apply(operation);
    ...
    int m = (l + r) >> 1;
    applyRange(cur -> left, l, m, ll, rr, operation);
    applyRange(cur -> right, m, r, ll, rr, operation);
    cur -> combine();
  }
