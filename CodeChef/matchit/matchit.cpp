...
int main () {
	...
	for (auto &c: cls) {
		...
		int st = 0;
		if (hasRmd) {
			point connection = point{xmin, isup ? 1 : n};
			vector<point> rmdPth2;
			if (c.pts[0].y == connection.y) {
				rmdPth2 = match(c.pts[0], connection, true);
			} else {
				rmdPth2 = matchsmart(c.pts[0], connection, xmin, xmax, (isup) ? ((c.pts.size() > 1) ? c.pts[1].y - 1 : n) : n);
			}
			if (rmdPth2.front() != connection) {
				reverse(rmdPth2.begin(), rmdPth2.end());
			}
			for (int i = rmdPth.back().x + 1; i < connection.x; i++) {
				rmdPth.push_back(point{i, connection.y});
			}
			rmdPth.insert(rmdPth.end(), rmdPth2.begin(), rmdPth2.end());
		...
		if (((int)c.pts.size() - st) % 2) {
			hasRmd = true;
			rmd = c.pts.back();
			point connection = point{xmax, isup ? n : 1};
			if (rmd.y == connection.y) {
				rmdPth = match(rmd, connection, true);
			} else {
				rmdPth = matchsmart(rmd, connection, xmin, xmax, (isup) ? n : (c.pts.size() > 1) ? c.pts[c.pts.size() - 2].y - 1 : n);
			}
			if (rmdPth.back() != connection) {
				reverse(rmdPth.begin(), rmdPth.end());
			}
		}
		isup = !isup;
	}
