...
class Solver {
  public:
    ...
    long long update(int k, int val) {
      ...
      long long bSum = blueSum.query(0, n);
      int bMax = otherBlueMax.query(0, n);
      int rMax = redMax.query(0, n);

      //printf("--> eSum: %lld, bSum: %lld, bMax: %d, rMax %d\n", eSum, bSum, bMax, rMax);
      //fflush(stdout);
      return eSum - bSum - max(bMax, rMax);
    }
...
void f() {
  ...
  long long ans = 1;
  for (int j = 0; j < e; j++) {
    long long cur = solver.update(i[j], w[j]) % mod;
    ans = (ans * cur) % mod;
  }
  printf("%lld\n", ans);
}
